package front 
{
	 ...
	 * @author umhr
	 */
	public class TriLine extends Sprite
	{
		private static var _instance:TriLine;
		public function TriLine(block:SingletonBlock){init();}
		public static function getInstance():TriLine{
			if ( _instance == null ) {_instance = new TriLine(new SingletonBlock());};
			return _instance;
		}
		...
		private function calc(x:Number, y:Number):void {
            for (var i:int = 0; i < 3; i++) {
                ...
                var r:Number = 20 * (1 + 1 * Math.sin(_count / 12));
                var mousePoint:Point = new Point(x + Math.cos(radian) * r , y + Math.sin(radian) * r);
